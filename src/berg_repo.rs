...
impl BergRepo {
    ...
    pub fn create_initial_structure(
        ...
        let _repo = git2::Repository::init(&self.path)?;
    ...
    pub async fn sync(&self, force: bool, flagdump: bool) -> anyhow::Result<()> {
        ...
        let _ctf = self.client.get_metadata().await?;
        ...
        let uid = player.id;
        let solved_challenges: Vec<String> = solves
            .iter()
            .filter(|s| s.player_id == uid)
            ...
            if done {
                ...
                if flag_file.exists() {
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        let flag_result = self.client.submit_flag(&challenge.name, flag).await?;
                        match flag_result {
                            ...
                            SubmitFlagResult::Incorrect => {
                                tracing::info!("incorrect flag for challenge {}", challenge.name);
                                tried_flags
                                    .entry(challenge.name.clone())
                                    .or_default()
                                    .insert(flag.to_string());
